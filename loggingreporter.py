...
class LoggingReporter():
    def __init__(self, trn, tst, model, rawdata_dir, img_dir, do_save_func=None):
    ...
    def on_epoch_begin(self, epoch):
        if self.do_save_func is not None and not self.do_save_func(epoch):
            ...
            self._batch_gradients = [ [] for _ in self.model.layers[1:] ] #model.layers[1:] to skip Input layer
    ...
    def on_batch_begin(self, batch_size):
    ...
    def on_epoch_end(self, d_loss_real, d_loss_fake, g_loss, epoch):
        if self.do_save_func is not None and not self.do_save_func(epoch):
            # Don't log this epoch
            return
